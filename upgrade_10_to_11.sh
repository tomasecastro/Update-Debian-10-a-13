...
# upgrade_10_to_11.sh
# Upgrade helper: Debian 10 (buster) -> Debian 11 (bullseye)
# Safe wrapper: creates backups, checks disk, replaces apt sources, runs upgrade steps.
# Usage: sudo ./upgrade_10_to_11.sh [--apply-upgrade] [--install-docker] [--install-vmware] [--yes]
...
Usage: sudo ./upgrade_10_to_11.sh [--apply-upgrade] [--install-docker] [--install-vmware] [--yes]
Options:
  --apply-upgrade   : perform the apt sources edit and upgrade steps
  --install-docker  : install Docker CE after upgrade
  --install-vmware  : install open-vm-tools
...
# docker metadata backup if available
if command -v docker >/dev/null 2>&1; then
  log "Saving docker metadata"
  docker ps -a --format '{{.ID}} {{.Names}} {{.Image}}' > "$BACKUP_DIR/docker-containers.txt" || true
  docker images --format '{{.Repository}}:{{.Tag}} {{.ID}}' > "$BACKUP_DIR/docker-images.txt" || true
  docker volume ls --format '{{.Name}}' > "$BACKUP_DIR/docker-volumes.txt" || true
fi
...
if (( avail_gb < 4 )); then
  err "Less than 4GB free; free space before upgrading or proceed with caution."
  if ! confirm "Continue anyway?"; then echo "Aborted."; exit 3; fi
fi
...
# optional installs after upgrade
